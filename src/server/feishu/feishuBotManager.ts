...
const LOG_FILE_CHUNK_BYTES = 25 * 1024 * 1024;
...
function buildLoadingTaskCard(message: TaskMessage): Record<string, unknown> {
...
function buildTaskCard(task: GatewayTask, outputMode: "structured" | "raw" | "both"): Record<string, unknown> {
...
function buildTaskLogCard(task: GatewayTask, logContent: string, fileCount: number): Record<string, unknown> {
...
function statusLabel(status: GatewayTask["status"]): string {
...
function isTerminalTaskStatus(status: GatewayTask["status"]): boolean {
  return status === "completed" || status === "failed" || status === "cancelled";
}

function isCancellableTaskStatus(status: GatewayTask["status"]): boolean {
  return status === "queued" || status === "running" || status === "waiting_approval";
}

function cardUpdateDelayMs(task: GatewayTask): number {
...
function buildTaskLogMarkdown(task: GatewayTask, logContent: string, part: number, totalParts: number): string {
...
async function readTaskLog(task: GatewayTask): Promise<string> {
  if (task.rawLogPath) {
    try {
...
function splitLogForFiles(text: string): string[] {
